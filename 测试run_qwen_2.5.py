...
tokenizer = AutoTokenizer.from_pretrained("model/Qwen/Qwen2.5-0.5B-Instruct")
# 实例化一个预训练的因果语言模型，并将其移动到指定的device上执行。
model = AutoModelForCausalLM.from_pretrained("model/Qwen/Qwen2.5-0.5B-Instruct").to(device)
# 设置对话的提示词。
prompt = "你好"
